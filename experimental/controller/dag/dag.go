...
func injectFinalizerEdges(dag *DAG) error {
	for _, node := range dag.Nodes {
		...
		// Patch). Definition, Ref, Watch, and Gauge nodes never produce managed
		...
		case graph.NodeTypeGauge:
			return fmt.Errorf("node %q cannot finalize %q: Gauge nodes do not manage resources", node.ID, node.Finalizes)
		}
