...
func (r *GraphReconciler) reconcileForEach(ctx context.Context, graph *unstructured.Unstructured, node graphpkg.Node, eval *evaluator, watcher *watches.GraphWatcher, prevState *forEachState) (*nodeOutput, error) {
	...
	for _, id := range currentOrder {
		...
		if childNodeType == graphpkg.NodeTypePatch {
			applied, err = r.applySSA(ctx, graph, evalMap, watcher, node.ID, graphpkg.NodeTypePatch, eval.effectiveGeneration, node.Lifecycle.ForceApply())
		} else {
			applied, err = r.applySSA(ctx, graph, evalMap, watcher, node.ID, graphpkg.NodeTypeTemplate, eval.effectiveGeneration, node.Lifecycle.ForceApply())
		}
		if err != nil {
