...
// Template, Patch, Gauge). Watch and ForEach return early — they handle
...
func reconcileNode(ctx context.Context, c *clusterAccess, rs *reconcileScope, node graphpkg.Node, eval *evaluator) (*nodeOutput, error) {
	...
	case graphpkg.NodeTypeGauge:
		if err := reconcileGauge(ctx, node, eval, rs.gaugeStore, rs.graphKey); err != nil {
			return nil, err
		}
	...
	// Post-dispatch readyWhen for Definition, Gauge, and Ref (no keys to return).
