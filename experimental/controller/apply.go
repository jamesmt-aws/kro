...
// as a side effect."
func (r *GraphReconciler) applySSA(ctx context.Context, graph *unstructured.Unstructured, evalMap map[string]any, watcher *watches.GraphWatcher, nodeID string, nodeType graphpkg.NodeType, generation int64, forceApply bool) (*unstructured.Unstructured, error) {
	// Template nodes always use ForceOwnership for SSA — the Graph is the sole
	// owner of template resources. Without this, any external mutation that
	// creates a second field manager permanently blocks SSA. Per 003-ownership.md:
	// "Template nodes own their resources."
	// Patch nodes only force when the user explicitly opts in via lifecycle.apply: Force.
	forceOwnership := forceApply || nodeType == graphpkg.NodeTypeTemplate
	...
	applied, err := r.ssaPatch(ctx, obj, graph, forceOwnership)
	if err != nil {
		return nil, err
	}
...
//     a different Graph's identity label, returns ErrFieldConflict unless the
//     user explicitly opted in via lifecycle.apply: Force.
//   - Patch: checks that the target resource exists.
